...
import {
  ...
  getAiTransactionSuggestionsService,
...
export const getAiTransactionSuggestionsController = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = req.user?._id;

    const data = await getAiTransactionSuggestionsService(userId);
